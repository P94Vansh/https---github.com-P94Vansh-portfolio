...
const ContactForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Send request to /api/send-mails
        const responseSendMail = await fetch(`${process.env.NEXT_PUBLIC_URL}api/send-mails`, {
            method: "POST",
            body: JSON.stringify(data),
            headers: myHeaders,
        });
    
        let resSendMail = await responseSendMail.json();
    
        // Send request to /api/Contact
        const responseContact = await fetch(`${process.env.NEXT_PUBLIC_URL}api/Contact`, {
            method: "POST",
            body: JSON.stringify(data),
            headers: myHeaders,
        });
    
        let resContact = await responseContact.json();
    
        // Check for success messages from both responses
        if (resSendMail.message === 'Email sent successfully' && resContact.success === true) {
            success();
        } else {
            failed();
        }
